...
$$ \begin{align*} \ln\{Z_{\mid y})\} &\geq \ln\{Z_q\} - \langle \ln\{ Z_{\mid y} p(x \mid y) \} \rangle_q + \langle \ln\{Z_q q_\lambda(x) \} \rangle_q \\
...
$$ D_{KL}\left( q_\lambda(x) \Big\|\, \frac{p(x, y)}{p(y)} \right) = D_{KL}(q_\lambda(x), p(x, y) ) + \ln\{ p(y) \},$$
...
such that globally minimising the variational free energy occurs precisely when we have a particular optimal value of $$\lambda$$, i.e., $$ q_\lambda(x) \approx p(x,y) $$, _and_ a maximum for $$-\ln\{p(y)\} $$.
