...
\ln\{Z_{\mid y})\} &\geq \ln\{Z_q\} + \left\langle \ln\{Z_{\mid y}\} + \ln\left\{ \frac{p(x,y)}{p(y)} \right\} \right\rangle_q - \langle \ln\{Z_q\} + \ln\{q_\lambda(x) \} \rangle_q\\
0 &\geq \left\langle \ln\left\{ \frac{p(x,y)}{p(y)} \right\} \right\rangle_q - \langle \ln\{q_\lambda(x) \} \rangle_q\\
0 &\geq - D_{KL}( q_\lambda(x) \|\, p(x,y) ) - \ln\{p(y)\}. \end{align*} $$
...
such that globally minimising the variational free energy occurs precisely when we have a particular optimal value of $$\lambda$$, i.e., $$ q_\lambda(x) \approx p(x,y) $$, _and_ a maximum for $$-\ln\{p(y)\} $$. If you wish to look at it a different way, the Bogoliubov inequality arises _because_ there is a distance between the trial and true distributions which is not necessarily zero, and minimising the KL divergence between these two things makes $$F_V$$ approach $$F$$ for some critical value of $$\lambda$$. Taking the fact that the KL divergence between $$q$$ and $$p_{\mid y}$$ is equivalent to that of $$q$$ and $$p_{,y}$$ plus the log term, minimising the variational free energy of $$q$$ and $$p_{\mid y}$$ is equivalent to minimising either (ideally both) terms.  
