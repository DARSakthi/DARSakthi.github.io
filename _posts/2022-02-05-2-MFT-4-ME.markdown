...
Wherein I discuss at length what puts the 'mean' in mean field theory, and why we minimise free energy to calculate it. Some more general comments on free energy are given, because I can't help myself. The proofs in the middle about the effective field, and at the very bottom about variational inference, both appear new, so that's nice. 
...
by essentially the same argument as above. If the true $$\hat H$$ is equal to this Hamiltonian, then the corrective term is minimised, and $$F_V \approx F$$. 
...
- We also mentioned that the function for $$m$$ which arises from finding magnetisation can be read as telling us how the system behaves in the non-interacting spins limit; that is, assuming the full free energy function is the original system and that MFT tells us what the 'right' simplification of the systemm is given non-interacting spins, the $$m$$ we produce is like the field that arises when passing to this limit. Now, $$m$$ has been chosen _a priori_ as an order parameter, so figuring out what such an order parameter looks like naturally tells us how magnetisation looks. Connections to Landau theory are incidental at best !  One could say it reassures us that our _model_ actually exhibits a phase transition, given this is a critical point of $$F_V(\lambda)$$. But once again, a crucial point is that the bound cannot reach across the arguments—minimising in $$m$$ brings $$F_V$$ close to $$F$$. When all is said and done, choosing $$m$$ as a control parameter is contingent on its eligibility to satisfy all of these criteria, as a useful simplification that still captures useful dynamics. So, if our model 'magnetises' (n.b. scare quotes) like $$m$$, we assume the actual system will too—at least, approximately so.

I'll provide one final remark, on Bayesian methods and variational inference. The Bogoliubov inequality appears in variational inference (even if it's not initially apparent, _everything_ in statistical mechanics is—you've guessed it—statistical). We know that the free energy functional is of critical importance in physics and statistics. Thermodynamical relationships follow from $$F$$. The partition function $$Z$$, which contains everything one needs to know about the system (it gives the probability measure over the entire state space) is $$e^{-\beta F}$$. The gradient of the free energy generates dynamics for a probabilistic system like the Fokker-Planck equation (or at least generate models of those dynamics in the case of a physical system, depending on the [ontological commitments](https://link.springer.com/article/10.1007/s10539-021-09807-0) one makes). Energy-based learning frameworks suggest that most traditional neural network models minimise free energy. The free energy principle suggests that any system can be understood as exhibiting states that minimise free energy. This is the reason why we want to approximate $$F$$ as closely as possible, in _any_ sort of system. Many such ideas rely on variational inference, which is prescribed in part by the Bogoliubov inequality. For the simple case of a trial density $$q(x)$$ and a true density $$p(x)$$ over states of a field, $$x$$, the KL divergence of these quantities is literally the Helmholtz free energy, with the entropy in microscopic terms and internal energy consisting of the true distribution (this should make sense based on the previous discussion about how optimisation is like doing work). Suppose both $$q$$ and $$p$$ are exponential distributions, in terms of $$\hat H_0$$ and $$\hat H$$, respectively. Putting everything into the KL divergence and simplifying it, we get 

$$ D_{KL}(q, p) = \beta \langle \hat H - \hat H_0) \rangle_0 + \ln\left\{\frac{Z}{Z_0}\right\}. $$

Since $$D_{KL} \geq 0$$, we in turn have 

$$ \ln\{Z\} \geq \ln\{Z_0\} - \beta \langle \hat H - \hat H_0) \rangle_0, $$ 

which implies the Bogoliubov inequality (proof in [Sak22], with an unfortunate but inconsequential misprint in the sign of one term and the direction of one bound. These counteract each other such that the expression as a whole is correct, which becomes apparent in the line immediately following). Procedurally, variational inference is typically a bit more complicated, in that we have a slightly harder problem. It is often the case that the true distribution is unknown or too complicated to be accessible even in principle—say, a difficult marginal distribution which is the posterior of some Bayesian estimation problem. In that case, we can't minimise the distance away from something we don't know, so we rewrite the Bogoliubov inequality using our new probability densities to get  

$$ \ln\{Z(x\mid y)\} \geq \ln\{Z_q(x; \lambda)\} - \beta \langle \hat H(x \mid y) - \hat H_q(x ; \lambda)) \rangle_q \iff D_{KL}( q(x;\lambda), p(x \mid y) \geq 0 $$ 

where we have also introduced some parameter $$\lambda$$ to the trial distribution $$q$$ (not a notational accident; this is indeed a control parameter for our trial Hamiltonian). Just as in mean field theory, this KL divergence is the variational free energy of $$q$$ as compared to $$p$$, which is greater than (or equal to) the free energy of $$p$$ in general. Again, though, this is usually not tractable. So, we do the following: using the fact that $$p$$ and $$q$$ are exponential distributions (by assumption), Bayes' theorem to expand the posterior, and linearity of the expectation throughout, we can rewrite the Bogoliubov inequality for $$q$$ away from $$p(x \mid y)$$ as 

$$ \ln\{Z_{\mid y})\} \geq \ln\{Z_q\} - \beta \langle \ln\{ Z_{\mid y} p(x \mid y) \} \rangle_q + \beta \langle \ln\{Z_q q_\lambda(x) \} \rangle_q $$
$$\ln\{Z_{\mid y})\} \geq \ln\{Z_q\} - \beta \left\langle \ln\left\{ Z_{\mid y} \frac{p(x,y)}{p(y)} \right\} \right\rangle_q + \beta \langle Z_q \ln\{q_\lambda(x) \} \rangle_q$$
$$0 \geq \ln\{Z_q\} - \beta \left\langle \ln\left\{ \frac{p(x,y)}{p(y)} \right\} \right\rangle_q + \beta \langle \ln\{q_\lambda(x) \} \rangle_q$$
$$0 \geq - \beta \left\langle \ln\left\{ \frac{p(x,y)}{p(y)} \right\} \right\rangle_q + \beta \langle \ln\{q_\lambda(x) \} \rangle_q$$
$$0 \geq - \beta D_{KL}( q_\lambda(x) \| p(x,y) ) + \ln\{p(y)\}$.$

Et voilà, the ELBO is secretly the Bogoliubov inequality. In particular, now, the distance between $$q_\lambda(x)$$ and $$p(x \mid y)$$ is controlled by not just $$\lambda$$, but $$\ln\{p(y)\}$$, which one could read as scoring the difference between our trial distribution $$q$$'s performance for the 'true' distribution $$p(x,y)$$ and the actual true distribution $$p(x \mid y)$$. That is to say, we have the identity 

$$ D_{KL}\left( q_\lambda(x), \frac{p(x, y)}{p(y)} \right) = D_{KL}(q_\lambda(x), p(x, y) ) + \ln\{ p(y) \},$$

such that 

$$ D_{KL}\left( q_\lambda(x), \frac{p(x, y)}{p(y)} \right) = \ln\{Z_{,y} - \ln\{Z_q\} + \beta \langle \hat H_{,y} - \hat H_q \rangle_q + \ln\{p(y)\} \geq 0. $$ 

This implies the full relation 
...
Such that we minimise the variational free energy precisely when we have a particular optimal value of $$\lambda$$, i.e., 

$$ q_\lambda(x) \approx p(x,y) $$


To recap, we went over A) why it is we minimise $$F_V$$ in MFT, B) how this is equivalent to approximating the system with a field of means, C) what minimising $$F_V$$ actually means, and D) what it tells us about other types of approximations in statistics. Mean field theory is one of my favourite things in statistical physics, mostly because it takes something that should be ugly—heuristic, even mindless approximation of something that we feel 'should' work—and makes it into something rather elegant, with important things to say about physics and statistics itself. Hope you're coming round to it as well.
