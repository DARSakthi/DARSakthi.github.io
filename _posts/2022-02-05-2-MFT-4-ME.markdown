...
such that globally minimising the variational free energy occurs precisely when we have a particular optimal value of $$\lambda$$, i.e., $$ q_\lambda(x) \approx p(x,y) $$, _and_ a minimum for $$-\ln\{p(y)\} $$. In particular, since $$\ln\{p(y)\}$$ is negative by assumption, there is a further error to our approximation induced by matching to the wrong density, i.e., we have

$$-\ln\{Z_{,y} \} \leq - \ln\{Z_q\} + \beta \langle \hat H_{,y} - \hat H_q \rangle_q + \ln\{p(y)\} \leq - \ln\{Z_q\} + \beta \langle \hat H_{,y} - \hat H_q \rangle_q. $$
