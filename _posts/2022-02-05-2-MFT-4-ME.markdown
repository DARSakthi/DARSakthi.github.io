...
Wherein I discuss at length what puts the 'mean' in mean field theory, and why we minimise free energy to calculate it. Some more general comments on free energy are given, because I can't help myself. The proof at the very bottom appears new, so that's nice. 
...
from which we can finally extract some insights (multiple, in fact). 

Firstly ... (DRAFT BELOW HERE)


Firstly, we have the promised result that this is equivalent to the Bogoliubov inequality 

extension of the Bogoliubov inequality:

$$ \ln\{Z_{\mid y}\}  + \beta \langle \hat H_{\mid y} - \hat H_q \rangle_q = \ln\{Z_{,y} + \beta \langle \hat H_{,y} - \hat H_q \rangle_q + \ln\{p(y)\} \geq 0, $$

$$ F_{\mid y} + F_q $$

$$ F \leq F_0 + \lambda \Delta \hat H \iff D_{KL}(q_\lambda(x), p(x,y) ) \geq - \ln\{ p(y) \} $$

by the previous argument ( $$D_{KL} \geq 0$$ ). Now, the distance between the variational free energy and the true free energy is controlled not just by $$\lambda$$, but $$-\ln\{p(y)\}$$. That is, we have


However, this implies we minimise the variational free energy precisely when we have a particular optimal value of $$\lambda$$, such that

$$ q_\lambda(x) \approx p(x,y) $$

now, the distance between $$q_\lambda(x)$$ and $$p(x \mid y)$$ is controlled by not just $$\lambda$$, but $$-\ln\{p(y)\}$$. 

Et voilà, the ELBO is secretly the Bogoliubov inequality. 
